...
public class ScannerServiceParser {
    ...
    /**
     * Checks if device is connect-able (as Android cannot get this information directly, we just 
     * check if it has GENERAL DISCOVERABLE or LIMITED DISCOVERABLE flag set) and has required 
     * service UUID in the advertising packet. The service UUID may be <code>null</code>.
     */
    ...
    /**
     * Decodes the device name from Complete Local Name or Shortened Local Name field in Advertisement 
     * packet. Usually if should be done by {@link BluetoothDevice#getName()} method but some phones
     * skips that, f.e. Sony X-peria Z1 (C6903) with Android 4.3 where getName() always returns 
     * <code>null</code>.
     */
